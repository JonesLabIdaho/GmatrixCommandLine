...
public:
	...
	bool p_move_peak_repeatedly;
	int p_peak_shift_interval;
	double p_trt0_peak_shift, p_trt1_peak_shift;
	int p_directional_selection_initial_gens;
	double p_opt0_var, p_opt1_var;
...
int parse_command_line_arguments(int arg_c, char* arg_v[], parameter_value_set &parm_set)
{
	...
	parm_set.p_move_peak_repeatedly = false;
	parm_set.p_peak_shift_interval = 0;
	parm_set.p_trt0_peak_shift = 0;
	parm_set.p_trt1_peak_shift = 0;
	parm_set.p_directional_selection_initial_gens = 0;
	parm_set.p_opt0_var = 0;
	parm_set.p_opt1_var = 0;
	...
	for (i = 1; i < arg_c - 1; i++)
	{
		...
		if (tstr1 == "--move_peak_repeatedly")
		{
			if (tstr2[0] == 't' || tstr2[0] == 'T')
				parm_set.p_move_peak_repeatedly = true;
		}

		if (tstr1 == "--trt0_peak_shift")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_trt0_peak_shift;
		}
		if (tstr1 == "--trt1_peak_shift")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_trt1_peak_shift;
		}
		if (tstr1 == "--peak_shift_interval")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_peak_shift_interval;
		}
		if (tstr1 == "--init_gens_directional_sel")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_directional_selection_initial_gens;
		}
		if (tstr1 == "--opt0_variance")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_opt0_var;
		}
		if (tstr1 == "--opt1_variance")
		{
			std::stringstream ss(tstr2);
			ss >> parm_set.p_opt1_var;
		}
...
public:
	...
	void calculate_means()
	{
		...
		for (i = 0; i < vN.size(); i++)
		{
			...
			meanOpt0 = meanOpt0 + vopt0[i];
			meanOpt1 = meanOpt1 + vopt1[i];
		...
		meanOpt0 = meanOpt0 / dNgens;
		meanOpt1 = meanOpt1 / dNgens;
...
private:
	...
	int DirectionalSelectionInitialGens;
	int PeakShiftInterval;
	double Trt0PeakShift;
	double Trt1PeakShift;
	double Opt0Var, Opt0StDev;
	double Opt1Var, Opt1StDev;
	bool MovePeakRepeatedly;
...
public:
	...
	void save_population_variables(int generation)
	{
		...
		if (generation == 0) // Output the header in generation zero
		{
			...
			outfile << "Gen,Opt0,Opt1,N,zbar0,zbar1,P00,P11,P12,r(P),gbar0,gbar1,G00,G11,G01,r(G)";
	...
	int getNumberOfInitialDSgens()
	{
		return DirectionalSelectionInitialGens;
	}
	...
	{
		sim_data.vopt0.push_back(Optimum[0]);
		sim_data.vopt1.push_back(Optimum[1]);
	...
	void save_stored_variables(std::string filename)
	{
		...
		{
			outfile << "\n" << i + 1;
			outfile << "," << sim_data.vopt0[i];
			outfile << "," << sim_data.vopt1[i];
	...
	void append_means_to_output_file(std::string filename)
	{
		...
		outfile << "\n" << "Mean:";
		outfile << "," << sim_data.meanOpt0;
		outfile << "," << sim_data.meanOpt1;
	...
	{
		m_rec.mean_list.push_back(sim_data.meanOpt0);
		m_rec.mean_list.push_back(sim_data.meanOpt1);
	...
	void save_parameter_values(std::string filename)
	{
		...
		// Moving Optimum Parameters
		outfile << "Moving_Optimum_Parameters:\n";
		outfile << "No_Initial_Gens_Moving_Opt:," << DirectionalSelectionInitialGens << "\n";
		outfile << "Peak_Shift_Interval," << PeakShiftInterval << "\n";
		outfile << "Trait0_Peak_Shift," << Trt0PeakShift << "\n";
		outfile << "Trait1_Peak_Shift," << Trt1PeakShift << "\n";
		outfile << "Trait0_Optimum_Var," << Opt0Var << "\n";
		outfile << "Trait1_Optimum_Var," << Opt1Var << "\n";
		if (MovePeakRepeatedly)
			outfile << "Move_Peak_Repeatedly:,true\n";
		else
			outfile << "Move_Peak_Repeatedly:,false\n";
	...
	void update_parameter_values(parameter_value_set &parm_set)
	{
		...
		// Moving Optimum Parameters
		DirectionalSelectionInitialGens = parm_set.p_directional_selection_initial_gens;
		PeakShiftInterval = parm_set.p_peak_shift_interval;
		Trt0PeakShift = parm_set.p_trt0_peak_shift;
		Trt1PeakShift = parm_set.p_trt1_peak_shift;
		Opt0Var = parm_set.p_opt0_var;
		Opt1Var = parm_set.p_opt1_var;
		MovePeakRepeatedly = parm_set.p_move_peak_repeatedly;
		Opt0StDev = sqrt(Opt0Var);
		Opt1StDev = sqrt(Opt1Var);
	...
	void update_optimum(int generation, bool initial_ds_gens)
	{
		if (MovePeakRepeatedly)
		{
			if (generation % PeakShiftInterval == 0)
			{
				Optimum[0] = Optimum[0] + Trt0PeakShift + randnorm(0, Opt0StDev);
				Optimum[1] = Optimum[1] + Trt1PeakShift + randnorm(0, Opt1StDev);
			}
		}
		else
		{
			if (generation == PeakShiftInterval && !initial_ds_gens)
			{
				Optimum[0] = Optimum[0] + Trt0PeakShift + randnorm(0, Opt0StDev);
				Optimum[1] = Optimum[1] + Trt1PeakShift + randnorm(0, Opt1StDev);
			}
