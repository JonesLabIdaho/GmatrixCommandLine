...
int main(int argc, char* argv[])
{
	...
	for (rep = 0; rep < number_of_replicates; rep++)
	{
		...
		// Run generations of directional selection
		for (ig = 0; ig < my_sim[rep].getNumberOfInitialDSgens(); ig++)
		{
			my_sim[rep].update_optimum(ig+1, true);
		...
		for (generations = 0; generations < my_sim[rep].getNumberOfGenerations(); generations++)
		{
			...
			{
				my_sim[rep].update_optimum(generations+1, false);
				my_sim[rep].selection(false);
				my_sim[rep].calculate_values_progeny();
				my_sim[rep].population_regulation();
				my_sim[rep].monogamy();
	...
	outfile << "Rep,Opt0,Opt1,N,zbar0,zbar1,P00,P11,P12,r(P),gbar0,gbar1,G00,G11,G01,r(G)";
